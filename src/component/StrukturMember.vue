...
// Kalau anggota divisi sedikit (misal DPO cuma 2 orang), gak usah animasi, tampilkan statis
const shouldAnimate = computed(() => selectedDivision.value.images.length > 4);
...
function animate() {
  if (!isHovered.value && shouldAnimate.value) {
          ...
          <div class="flex bg-transparent rounded-md flex-col items-center mx-2 flex-shrink-0 hover:scale-105" :style="{ width: imageWidth + 'px' }">
            ...
            <div class="flex items-center justify-center gap-2 w-full">
              <div>
                ...
                <p v-if="img.jabatan" class="mb-3 text-center text-xs font-normal text-holy/50">
                  {{ img.jabatan }}
                </p>
              </div>
              <a v-if="img.instagram" :href="img.instagram" target="_blank" rel="noopener" class="ml-2 text-pink-500 hover:text-pink-600 transition-colors" title="Instagram">
